...
# This table represents the Output Share of Natural Resources in percent for the year 2000
...
"""
The function `delete_figure_1()` deletes the `pdf` file containing
the replication result of Figure 1, if the present working directory has a folder `output` containing it.
"""
...
function create_figure_2()
    ...
    for (quartile, style, color) in zip(unique_quartiles, line_styles, colors)
        ...
        plot!(  quartile_data.year, quartile_data.phi_NR_avg,
